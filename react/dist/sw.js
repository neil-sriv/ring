...
const C = (a) =>
  [f.prefix, a, f.suffix].filter((e) => e && e.length > 0).join("-")
const O = (a) => {
...
async function j(a, e, t, s) {
  ...
  const i = await a.keys(e, r)
  for (const c of i) {
    const o = P(c.url, t)
    if (n === o) return a.match(c, s)
  }
}
class H {
  constructor() {
...
class V {
  ...
  async cacheMatch(e) {
    ...
    const i = await this.getCacheKey(t, "read")
    const c = Object.assign(Object.assign({}, r), { cacheName: n })
  ...
  async cachePut(e, t) {
    ...
    const o = await self.caches.open(i)
    const h = this.hasCallback("cacheDidUpdate")
    const g = h ? await j(o, n.clone(), ["__WB_REVISION__"], c) : null
    try {
...
class K {
  ...
  handleAll(e) {
    ...
    const r = new V(this, { event: t, request: s, params: n })
    const i = this._getResponse(r, s, t)
    const c = this._awaitComplete(i, r, s, t)
    return [i, c]
  }
    ...
    try {
      ...
      if (r instanceof Error) {
        for (const i of e.iterateCallbacks("handlerDidError"))
          if (((n = await i({ error: r, event: s, request: t })), n)) break
      }
      if (!n) throw r
    }
...
class G {
  ...
  addToCacheList(e) {
    ...
    for (const s of e) {
      ...
      const i = typeof s !== "string" && s.revision ? "reload" : "default"
...
This is generally NOT safe. Learn more at https://bit.ly/wb-precache`
  ...
  install(e) {
    return k(e, async () => {
      ...
      for (const [r, i] of this._urlsToCacheKeys) {
        ...
        const o = this._urlsToCacheModes.get(r)
        const h = new Request(r, {
          integrity: c,
          cache: o,
  ...
  activate(e) {
    return k(e, async () => {
      ...
      const n = new Set(this._urlsToCacheKeys.values())
      const r = []
      for (const i of s) n.has(i.url) || (await t.delete(i), r.push(i.url))
...
const x = "GET"
const R = (a) => (a && typeof a === "object" ? a : { handle: a })
class w {
  constructor(e, t, s = x) {
...
se(
  [
    { revision: null, url: "assets/index-fJdqG-Cr.js" },
    { revision: null, url: "assets/workbox-window.prod.es5-B9K5rw8f.js" },
    { revision: "e3d2d7d372ffe97cc3f4b9553be98c8c", url: "index.html" },
    {
      revision: "13f80865c8ba0f62f140959fd73bb703",
      url: "assets/images/pwa-192x192.png",
    },
    {
      revision: "5d5b77f8430b2a13e0b74f83eade54a3",
      url: "assets/images/pwa-512x512.png",
    },
    {
      revision: "8930aca33dcc645ec8661da0a56be45b",
      url: "manifest.webmanifest",
    },
  ] || [],
)
T(
  ({ request: a }) => a.url.startsWith("https://api.yourdomain.com/"),
  new ne({ cacheName: "api-cache" }),
)
caches.keys().then((a) => {
  for (const e of a) caches.delete(e)
})
self.addEventListener("push", (a) => {
  if (a.data) {
    const e = a.data.json()
    console.log("Push event data:", e),
      a.waitUntil(
...
self.addEventListener("notificationclick", (a) => {
  a.notification.close(),
    a.notification.data && a.waitUntil(clients.openWindow("ring"))
})
