...
def upgrade() -> None:
    ...
    session.execute(sa.text("UPDATE letter SET status = 'SENT' WHERE status IS NULL"))
    op.alter_column("letter", "status", existing_type=sa.VARCHAR(), nullable=False)
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.alter_column("letter", "status", existing_type=sa.VARCHAR(), nullable=True)
    session = sa.orm.Session(bind=op.get_bind())
